...
import { useDeleteOrder } from "../handlers/OrderHandler";
const { Option } = Select;
...
const Order = () => {
      ...
      children: [
        {
          ...
          render: (text, record) =>
            record.lineItems
              ?.map((item) => (
                <span key={item.productId}>{item.productId}</span>
              ))
    ...
    },
    {
      ...
      render: (_, record) => (
        <div className="action-buttons">
          ...
          <button
            onClick={() => handleDeleteOrder(record.id, record.recurringStatus)}
          >
            <FaTrash />
          </button>
        </div>
  ...
  const handleEdit = (record) => {
    ...
    form.setFieldsValue({
      ...
      deliveryOrderAttributes: {
        plannedAt: record.deliveryOrder.plannedAt
          ? moment(record.deliveryOrder.plannedAt)
          : null,
        completedAt: record.deliveryOrder.completedAt
          ? moment(record.deliveryOrder.completedAt)
          : null,
        consumerOutletId: record.deliveryOrder.consumerOutletId,
        lineItemsAttributes: record.lineItems || [],
      },
    });
  };
  ...
  const onFinish = (values) => {
    // values=values.lineItemsAttributes.delete()
    // delete.orderGroups.lineItemsAttributes
  ...
  const handleCreateOrder = async (values) => {
    try {
      ...
      if (missingStatusItems.length > 0) {
        throw new Error(
          `All line items must have a status. Missing: ${JSON.stringify(
            missingStatusItems
          )}`
        );
      }
      const formattedValues = {
        ...
        deliveryOrderAttributes: {
          ...
          plannedAt: values.deliveryOrderAttributes.plannedAt
            ? values.deliveryOrderAttributes.plannedAt.toISOString()
            : null,
          completedAt: values.deliveryOrderAttributes.completedAt
            ? values.deliveryOrderAttributes.completedAt.toISOString()
            : null,
  ...
  const handleUpdateOrder = async (values) => {
    try {
      const lineItemsAttributes = Array.isArray(values.lineItems)
        ? values.lineItems
        : [];
      ...
      const formattedValues = {
        ...
        deliveryOrderAttributes: {
          consumerOutletId:
            values.deliveryOrderAttributes?.consumerOutletId || null,
  ...
  return (
    <div>
      <h1>Order List</h1>
      <Button type="primary" onClick={handleAddOrder} icon={<PlusOutlined />}>
        Add Order
      </Button>
      <Table
        columns={columns}
      ...
      >
        {modalMode === "view" ? (
          <div>
            ...
            <p>
              <strong>Consumer ID:</strong> {selectedOrder?.consumerId}
            </p>
            <p>
              <strong>Tenant ID:</strong> {selectedOrder?.tenantId}
            </p>
          </div>
        ) : (
          <Form
          ...
          >
            ...
            <Form.Item
              label="Planned At"
              name={["deliveryOrderAttributes", "plannedAt"]}
              rules={[{ required: true }]}
            >
            ...
            <Form.Item
              label="Completed At"
              name={["deliveryOrderAttributes", "completedAt"]}
              rules={[{ required: true }]}
            >
              <DatePicker format="YYYY-MM-DD HH:mm:ss" showTime required />
            </Form.Item>
            <Form.Item
              label="Consumer Outlet"
              name={["deliveryOrderAttributes", "consumerOutletId"]}
              rules={[{ required: true }]}
            >
              <Select
